...
from pymongo import IndexModel, AsyncMongoClient
...
class BaseImageCache(Document):
    date: int = Field(
        default_factory=lambda: int(str(datetime.now().date()).replace("-", ""))
    )
...
async def init_db(host: str, port: int, user: str, password: str):
    ...
    mongo_client = AsyncMongoClient(
        connection_string, unicode_decode_error_handler="ignore"
    )
    await init_beanie(
        database=mongo_client["ChatImitateDB"],
        document_models=[
            BotConfigModule,
            GroupConfigModule,
            UserConfigModule,
